...
- We will create a file named `linker.ld`. It uses a specific declarative syntax with the following rules:
  ...
  6. **The Data Section:** Define a `.data` section. Align to 4K. Inside, place `*(COMMON)` and then `*(.bss)`. This holds uninitialized variables and, crucially, the memory we will use for our kernel's stack.
### Pitfalls:
